...
(function() {
    ...
    const Share = {
        ...
        async shareWin() {
            ...
            const canShareFiles = navigator.canShare && 
                navigator.canShare({ files: [new File(['test'], 'test.png', { type: 'image/png' })] });
            
            if (canShareFiles) {
                // Mobile: Generate and share image
                UI.showToast('Erstelle Bild...', 'info');
                
                try {
                    const imageBlob = await this.generateBoardImage();
                    await this.shareWithImage(imageBlob);
                } catch (e) {
        ...
        downloadImage(imageBlob) {
            ...
            a.href = url;
            a.download = 'bahn-bingo-sieg.png';
            ...
            navigator.clipboard.writeText(text + '\n' + window.location.href)
                .then(() => UI.showToast('Bild heruntergeladen & Text kopiert! 📥', 'success'))
                .catch(() => UI.showToast('Bild heruntergeladen! 📥', 'success'));
        },
        ...
        async generateBoardImage() {
            ...
            // Decorative circles
            ctx.fillStyle = 'rgba(236, 28, 36, 0.1)';
            ctx.beginPath();
            ctx.arc(100, 200, 150, 0, Math.PI * 2);
            ctx.fill();
            
            ctx.fillStyle = 'rgba(236, 28, 36, 0.08)';
            ctx.beginPath();
            ...
            ctx.fillText('🏆 BINGO!', width / 2, 100);
            
            // Username
            ctx.fillStyle = '#ff6b6b';
            ctx.font = 'bold 36px Arial, sans-serif';
            ctx.fillText(state.user.username, width / 2, 160);
            
            // Board container
            const boardX = 60;
            const boardY = 200;
            const boardWidth = width - 120;
            const boardHeight = 850;
            
            // Board background
            ctx.fillStyle = '#16213e';
            this.roundRect(ctx, boardX, boardY, boardWidth, boardHeight, 20);
            ctx.fill();
            
            // BINGO header
            const cellWidth = (boardWidth - 40) / 5;
            const headerHeight = 70;
            const letters = ['B', 'I', 'N', 'G', 'O'];
            
            const redGrad = ctx.createLinearGradient(boardX, boardY, boardX + boardWidth, boardY);
            redGrad.addColorStop(0, '#ec1c24');
            redGrad.addColorStop(1, '#ff6b6b');
            
            letters.forEach((letter, i) => {
                const x = boardX + 20 + i * cellWidth;
                ctx.fillStyle = redGrad;
                this.roundRect(ctx, x, boardY + 15, cellWidth - 8, headerHeight, 8);
                ctx.fill();
                
                ctx.fillStyle = '#ffffff';
                ctx.font = 'bold 42px Arial, sans-serif';
                ctx.textAlign = 'center';
                ctx.fillText(letter, x + cellWidth / 2 - 4, boardY + 60);
            });
            
            // Grid cells
            const gridY = boardY + headerHeight + 25;
            const cellHeight = (boardHeight - headerHeight - 50) / 5;
            
            const greenGrad = ctx.createLinearGradient(0, gridY, 0, gridY + cellHeight * 5);
            greenGrad.addColorStop(0, '#00d26a');
            greenGrad.addColorStop(1, '#00a854');
            
            state.board.forEach((cell, index) => {
                const row = Math.floor(index / 5);
                const col = index % 5;
                const x = boardX + 20 + col * cellWidth;
                ...
                // Cell text
                ctx.fillStyle = state.marked.has(index) || cell.isFreeSpace ? '#ffffff' : '#aaaaaa';
                if (cell.isFreeSpace) ctx.fillStyle = '#333333';
                
                ctx.font = '18px Arial, sans-serif';
                ctx.textAlign = 'center';
                
                // Word wrap for cell text
                ...
                const lineHeight = 22;
                const textY = y + h / 2 - (lines.length - 1) * lineHeight / 2;
                
                lines.forEach((line, i) => {
                    ctx.fillText(line, x + w / 2, textY + i * lineHeight);
                });
                
                // Checkmark for marked cells
                if (state.marked.has(index) && !cell.isFreeSpace) {
                    ctx.fillStyle = '#ffffff';
                ...
                if (cell.isFreeSpace) {
                    ...
                    ctx.fillText('⭐', x + w - 20, y + 24);
                }
            });
            
            // Stats section
            const statsY = boardY + boardHeight + 30;
            
            ctx.fillStyle = '#16213e';
            this.roundRect(ctx, boardX, statsY, boardWidth, 100, 16);
            ctx.fill();
            
            const stats = state.user.stats;
            const statItems = [
                { label: 'SERIE', value: stats.currentStreak, icon: '🔥' },
                { label: 'SIEGE', value: stats.wins, icon: '🏆' },
                { label: 'QUOTE', value: Math.round((stats.wins / Math.max(stats.gamesPlayed, 1)) * 100) + '%', icon: '📊' }
            ];
            
            const statWidth = boardWidth / 3;
            statItems.forEach((stat, i) => {
                const x = boardX + i * statWidth + statWidth / 2;
            ...
            ctx.textAlign = 'center';
            ctx.fillText('#BahnBingo 🚂', width / 2, height - 60);
            
            ctx.fillStyle = '#555555';
            ctx.font = '20px Arial, sans-serif';
            ctx.fillText('Spiel mit! → bahn-bingo.app', width / 2, height - 25);
        ...
        async shareWithImage(imageBlob) {
            const stats = state.user.stats;
            const text = `🏆 BINGO!\n\n` +
                `Ich habe bei Bahn-Bingo gewonnen!\n` +
                `🔥 Serie: ${stats.currentStreak}\n` +
            ...
            if (navigator.canShare && navigator.canShare({ files: [new File([imageBlob], 'bahn-bingo.png', { type: 'image/png' })] })) {
                const file = new File([imageBlob], 'bahn-bingo.png', { type: 'image/png' });
                ...
                try {
                    ...
                    if (e.name !== 'AbortError') {
                        console.log('Share with file failed, trying download');
                    } else {
            ...
            UI.showToast('Bild heruntergeladen! 📥', 'success');
    ...
    function setupEventListeners() {
        ...
        document.getElementById('profileBackBtn').addEventListener('click', () => Profile.hide());
        document.getElementById('editNameBtn').addEventListener('click', () => Profile.showEditName());
        document.getElementById('cancelNameBtn').addEventListener('click', () => Profile.hideEditName());
        document.getElementById('saveNameBtn').addEventListener('click', () => Profile.saveName());
        document.getElementById('shareProfileBtn').addEventListener('click', () => Profile.share());
        document.getElementById('exportDataBtn').addEventListener('click', () => Profile.exportData());
        document.getElementById('importDataBtn').addEventListener('click', () => Profile.showImportModal());
        document.getElementById('resetStatsBtn').addEventListener('click', () => Profile.confirmResetStats());
        document.getElementById('logoutBtn').addEventListener('click', () => Profile.confirmLogout());
        
        // Confirm modal
        document.getElementById('confirmCancel').addEventListener('click', () => Profile.hideConfirmModal());
        
        // Import modal
        document.getElementById('importCancel').addEventListener('click', () => Profile.hideImportModal());
        document.getElementById('importOk').addEventListener('click', () => Profile.importData());
    }
    ...
    const Profile = {
        ...
        render() {
            ...
            if (state.user.createdAt) {
                ...
                document.getElementById('profileSince').textContent = 'Heute';
            }
            
            // Quick stats
            document.getElementById('profileWins').textContent = stats.wins;
            document.getElementById('profileStreak').textContent = stats.bestStreak;
            document.getElementById('profileStars').textContent = stats.totalStars || 0;
        },

        showEditName() {
            document.getElementById('editNameSection').classList.remove('hidden');
            document.getElementById('newNameInput').value = state.user.username;
        ...
        saveName() {
            ...
            this.render();
            UI.updateStats();
            UI.showToast('Name geändert! ✅', 'success');
        },

        share() {
            const stats = state.user.stats;
            const text = `🚂 Mein Bahn-Bingo Profil\n\n` +
                `👤 ${state.user.username}\n` +
                `🏆 ${stats.wins} Siege\n` +
                `🔥 Beste Serie: ${stats.bestStreak}\n` +
                `⭐ ${stats.totalStars || 0} Sterne\n\n` +
                `Spiel mit! #BahnBingo`;
        ...
        exportData() {
            ...
            navigator.clipboard.writeText(encoded)
                .then(() => {
                    UI.showToast('Export-Code kopiert! 📋', 'success');
                })
                .catch(() => {
                    // Fallback: show in prompt
                    prompt('Kopiere diesen Code:', encoded);
                });
        },
        ...
        importData() {
            ...
            if (!code) {
                UI.showToast('Bitte Code eingeben', 'error');
                return;
            }
            ...
            try {
                ...
                if (!decoded.user || !decoded.user.stats) {
                    throw new Error('Invalid format');
                }
                
                // Validate imported stats
                if (!AntiCheat.validateStats(decoded.user.stats)) {
                    UI.showToast('Ungültige Daten', 'error');
                    return;
                }
                ...
                this.render();
                UI.updateStats();
                UI.showToast('Daten importiert! ✅', 'success');
                
            } catch (e) {
                UI.showToast('Ungültiger Import-Code', 'error');
            }
        },
        ...
        confirmResetStats() {
            this.showConfirmModal(
                ...
                'Alle deine Siege, Serien und Achievements werden gelöscht. Dein Name bleibt erhalten.',
                () => {
                    ...
                    this.render();
                    UI.updateStats();
                    UI.showToast('Statistiken zurückgesetzt', 'success');
                }
            );
        },
